<template>
  <ion-page>
    <ion-content class="">
      <div
        class="flex justify-center items-center p-4 min-h-screen text-white bg-gradient-to-br from-gray-900 via-black to-gray-800">
        <!-- Contenedor principal -->
        <div
          class="p-6 w-full max-w-md rounded-xl border border-gray-700 shadow-xl backdrop-blur-md bg-gray-800/50 shadow-purple-900/20">
          <!-- Header futurista -->
          ...
          <div class="space-y-6">
            ...
            <!-- To this -->
<button class="flex justify-between items-center w-full text-left" @click="resetModalOpened = true">
  <span class="font-medium text-purple-300">Reestablecer contraseña desde la app</span>
  <ion-icon :icon="chevronForwardOutline" class="text-gray-400"></ion-icon>
</button>
            </div>

            <div class="p-4 rounded-lg border border-gray-600 transition-all bg-gray-700/30 hover:bg-gray-700/50">
              <button class="flex justify-between items-center w-full text-left" @click="resetModalOpenedApp = true">
  <span class="font-medium text-purple-300">Reestablecer contraseña desde el correo</span>
  <ion-icon :icon="chevronForwardOutline" class="text-gray-400"></ion-icon>
</button>
            </div>

            <div class="p-4 rounded-lg border border-gray-600 transition-all bg-gray-700/30 hover:bg-gray-700/50">
              <button class="flex justify-between items-center w-full text-left" @click="cerrarSesion">
      ...
      <!--Reset via email-->
      <ion-modal :is-open="resetModalOpened" @didDismiss="resetModalOpened = false" mode="ios">
        <div class="ion-padding">
          <div
            class="p-6 w-full max-w-md rounded-xl border border-gray-700 shadow-xl backdrop-blur-md bg-gray-800/80 shadow-purple-900/30">
            <!-- Header del modal -->
            <header class="mb-6 text-center">
              ...
              <p class="mt-2 text-xs text-gray-400">
                Ingrese su nueva contraseña para actualizarla desde la app
              </p>
            </header>

            <!-- Formulario -->
            <form @submit.prevent="enviarEnlaceReestablecimiento" class="space-y-5">
              <!-- Campo: Update Password -->
              <div class="space-y-2">
                <label for="email" class="block text-sm font-medium text-gray-300">Ingrese su nueva contraseña</label>
                <div class="relative">
                  <input type="password" id="email" v-model="newPassword"
      ...
      <!--Reset via app-->
      <ion-modal :is-open="resetModalOpenedApp" @didDismiss="resetModalOpenedApp = false" mode="ios">
        <div class="ion-padding">
          <div
            class="p-6 w-full max-w-md rounded-xl border border-gray-700 shadow-xl backdrop-blur-md bg-gray-800/80 shadow-purple-900/30">
            <!-- Header del modal -->
            ...
            <!-- Formulario -->
            <form @submit.prevent="updatePasswordApp" class="space-y-5">
              <!-- Campo: Correo -->
              <div class="space-y-2">
                <label for="email" class="block text-sm font-medium text-gray-300">Correo Electrónico</label>
                <div class="relative">
                  <input type="email" id="email" v-model="correoReestablecer"
                    class="px-4 py-3 w-full placeholder-gray-400 text-white rounded-lg border border-gray-600 transition-all outline-none bg-gray-700/50 focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                    placeholder="ejemplo@losestafadores.com" required />
                </div>
              </div>
              ...
              <div class="flex flex-col pt-2 space-y-3">
                ...
                <button type="button"
                  class="!px-4 !py-3 w-full font-medium text-gray-300 !rounded-sm !border !border-gray-600 transition-all bg-gray-700/50 hover:bg-gray-700"
                  @click="resetModalOpenedApp = false">
                  Cancelar
                </button>
              </div>
            </form>
...
const resetModalOpenedApp = ref(false);

const updatePasswordApp = () => {
  console.log("Actualizando contraseña:", newPassword.value);
  try {
    if (!auth.currentUser) {
    ...
    resetModalOpenedApp.value = false;
  } catch (error) {
    notyf.error('Error al actualizar la contraseña');
    resetModalOpenedApp.value = false;
